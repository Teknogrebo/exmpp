...
response(KeyVals, User, Passwd, Nonce, AuthzId, A2Prefix) ->
    ...
    A1 = case AuthzId of
	     "" ->
		 binary_to_list(
		   compat:crypto_md5(User ++ ":" ++ Realm ++ ":" ++ Passwd)) ++
		     ":" ++ Nonce ++ ":" ++ CNonce;
	     _ ->
		 binary_to_list(
           compat:crypto_md5(User ++ ":" ++ Realm ++ ":" ++ Passwd)) ++
    ...
    T = hex(binary_to_list(compat:crypto_md5(A1))) ++ ":" ++ Nonce ++ ":" ++
	NC ++ ":" ++ CNonce ++ ":" ++ QOP ++ ":" ++
	hex(binary_to_list(compat:crypto_md5(A2))),
    hex(binary_to_list(compat:crypto_md5(T))).
